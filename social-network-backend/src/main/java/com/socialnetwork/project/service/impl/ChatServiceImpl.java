...
public class ChatServiceImpl implements ChatService {
    ...
    public Chat getSystemChatByUserIdOrElseCreate(Long userId) {
        ...
        Optional<Chat> chat = chatRepository.findChatByUsers(userId, systemUserId);
        if(!chat.isPresent()) {
            Chat newChat = new Chat()
                    .toBuilder()
                    .users(Set.of(
                            userRepository.findById(userId).orElseThrow(),
                            userRepository.findById(systemUserId).orElseThrow()
                    ))
                    .build();
    ...
    private ChatMessageStatusDTO convertToChatMessageStatusDTO(Long chatId, Message message, MessageStatus messageStatus) {
        return new ChatMessageStatusDTO()
                .toBuilder()
                .chatId(chatId)
