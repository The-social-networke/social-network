...
public class MessageServiceImpl implements MessageService {
    ...
    public Message readMessage(MessageReadDTO dto) {
        ...
        message.getMessageReads().add(userRepository.findById(dto.getUserId()).orElseThrow());
        return messageRepository.save(message)
                .toBuilder()
                .messageStatus(MessageStatus.UPDATED)
                .build();
    ...
    public Message toggleLikeMessage(MessageLikeDTO dto) {
        ...
        if(isLikeDTO) {
            message.getMessageLikes().add(userRepository.findById(dto.getUserId()).orElseThrow());
        }
        else {
            message.getMessageLikes().remove(userRepository.findById(dto.getUserId()).orElseThrow());
        }
        return messageRepository.save(message)
                .toBuilder()
                .messageStatus(MessageStatus.UPDATED)
